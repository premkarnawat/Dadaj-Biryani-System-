...
// Cart icon position (top-right button in TopNav) — approximate
const CART_POSITION = { x: window?.innerWidth ? window.innerWidth - 80 : 1200, y: 32 };
...
export default function DishCard({
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    // Fly animation toward cart
    await controls.start({
      x: [0, CART_POSITION.x - startX],
      y: [0, CART_POSITION.y - startY],
      scale: [1, 0.15],
      opacity: [1, 0],
      ...
      <Link href={`/dishes/${id}`}>
        ...
        <div className="p-3.5">
          ...
          <div className="flex items-center justify-between">
            <span className="font-bold text-primary-600 text-base">{formatPrice(price)}</span>
            <motion.button
